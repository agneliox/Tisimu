...
public class HymnalRepository {
    ...
    public HymnalRepository(Application application) {
        ...
        // Force check and insert sample data if needed
        checkAndInsertSampleData();
    }

    private void checkAndInsertSampleData() {
        new Thread(() -> {
            int count = database.hymnalDao().getCountSync();
            if (count == 0) {
                Log.d("HymnalRepository", "No hymnals found, inserting sample data");
                insertSampleHymnals();
            } else {
                Log.d("HymnalRepository", "Found " + count + " hymnals already in database");
            }
        }).start();
    }

    private void insertSampleHymnals() {
        ...
        Log.d("HymnalRepository", "Sample hymnals inserted");
    }
